...
1. **Resolve issue** — Resolve key/URL → single active ISSUES row → ask once; fetch Task, children, Story, PLAN/BUG/TWEAK/ITERATE, and linked PR. Done when one Task and its delivery state are identified, including an already-Done result.
...
| Evidence | Remaining |
|----------|-----------|
| No ready-to-build artifact | **Stop** — `/define`, `/bug`, `/tweak`, or `/iterate` first |
