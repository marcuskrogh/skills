...
`/ship` finalizes whatever is still left after define / bug / tweak / iterate-ready.
It is not limited to “clean review → merge”.

```text
detect stage
  → not ready-to-build → stop; tell user /define|/bug|/tweak|/iterate
...
|--------------|-----------|
| After define / bug / tweak (To Do, plan/bug/tweak ready) | implement → review-fix → closeout |
...
1. **Pre-merge continuity (PR still open)** — commit and push on the delivery branch:
   - PLAN / BUG / TWEAK / ITERATE — shipped / **Next: Done** + PR link
