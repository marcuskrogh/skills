...
| **iterate** | **New** Task; link ITERATE.md; Relates → prior | In Progress (implement/test); **In Review** after harden or when harden skipped | Prior + new Task comments; **Next** first closeout step (`/test`); ISSUES | — (ship closes) |
...
| **implement** | May add missing Sub-tasks (incl. Testing); **reuse** delivery | Task → In Progress; Sub-tasks → Done as finished; stay **In Progress** after Build unless Next is `/review-fix` | Session + packages + **same** PR + **Next**; ISSUES | Sub-tasks only — not parent |
| **implement** (fix-forward) | — | In Progress if needed → **In Review** | Threads addressed + **Next**; ISSUES | — |
| **test** | May add Testing Sub-tasks | Stay **In Progress**; **In Review** when Next is `/review-fix` (harden skipped) | Testing outcome + **Next**; ISSUES | — |
