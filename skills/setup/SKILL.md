...
description: >-
  ...
  before first explore/define/bug/tweak/refine.
...
4. **Verify and hand off** — Check provider credentials ([../tracker/reference.md](../tracker/reference.md)); report path, scope, tracker, gaps, and **Next** (`/explore`, `/bug`, `/tweak`, `/refine`, or none). Done when the user has the Next cue. Commit only on ask — never commit a global workspace file.
