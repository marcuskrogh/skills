...
description: >-
  ...
  spec locks, representability), adversarial coverage, and testability. Adds or
...
1. **Resolve delivery** — Resolve the Task, spec, Workflow binding, and delivery PR. If `test.mode=skip` (docs-only or explicit user ask) **and** class is not adopt, persist Next `/restructure` and stop. Adopt / `ADOPT.md` → run the pass. Done when the PR is checked out in this session's working tree or skip is recorded.
...
4. **Track and hand off** — Stay **In Progress**. Comment the testing outcome (coverage/CRAP/spec locks considered), persist **Next** `/restructure`. Done when Task, PR, mirror, and user report agree.
