...
- **Tested delivery.** Behavioural packages include/update tests in-package (or a Testing package before verify); bug fixes and behaviour-changing tweaks include regression coverage; refinements verify behaviour is unchanged (non-regression); reworks verify the parity bar via baseline vs candidate comparative evaluation before accept; injectable seams; touched-area suite stays honest and green.
