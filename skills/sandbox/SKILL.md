...
description: >-
  ...
  UI, method, or bench. Produces SANDBOX.md and a runnable harness outside
...
input** on the delivery branch (no sandbox PR). The harness is **representative**
...
4. **Iterate one turn** — Apply the operator's latest delta (or the initial extract); run the harness; capture the inspectable on the manager (harness files, RecordScreen, Read); present it; ask one question: accept and promote, name a delta, or end sandbox-only. Done when the inspectable is shown and that question is asked.
